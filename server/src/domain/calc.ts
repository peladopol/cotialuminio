...
function areaM2(a_mm: number, h_mm: number) { return (a_mm/1000)*(h_mm/1000); }
function perimetroMl(a_mm: number, h_mm: number) { return 2*((a_mm/1000)+(h_mm/1000)); }
...
function vidrioIdFromToken(db: CostDB, token: string): string {
  ...
  if (t === "33" || t === "L33" || t === "LAM33") {
    const cand = db.vidrios.find(v => v.tipo === "LAM" && (/33|3_3/.test(normId(v.id)) || String((v).configuracion ?? "").toUpperCase().includes("3+3")));
...
function parseDVHId(db: CostDB, dvhId: string): { interior: string; exterior: string; camara_mm: number } {
  // Acepta:
  ...
  const readGlass = (idx: number): { token: string; next: number } => {
    ...
    if (parts[idx] === "3" && parts[idx + 1] === "3") return { token: "L33", next: idx + 2 };
...
function kg_m(db: CostDB, codigo: string): number {
  ...
  if (v <= 0) throw new Error(`kg/m inválido para perfil ${codigo}. Cargalo en Excel (hoja Perfiles).`);
  return v;
}

function kgPerfil(db: CostDB, codigo: string, largo_mm: number, cantidad: number, validateLength = true): number {
  if (validateLength) {
    const barra = db.parametros.aluminio_barra_mm ?? 6000;
    if (largo_mm > barra) throw new Error(`Corte de perfil ${codigo} = ${Math.round(largo_mm)} mm supera barra de ${barra} mm.`);
    if (largo_mm <= 0) throw new Error(`Corte inválido para perfil ${codigo}: ${largo_mm} mm`);
  }
...
function validateRangos(db: CostDB, i: QuoteInput) {
  ...
  if (a < minA || h < minH) throw new Error(`Medida fuera de rango para ${i.tipologia}. Mínimo: ${minA} x ${minH} mm.`);
  if (a > maxA || h > maxH) throw new Error(`Medida fuera de rango para ${i.tipologia}. Máximo: ${maxA} x ${maxH} mm.`);
}
...
function validateVidrioSheet(db: CostDB, pieces: Array<{w: number; h: number; label: string}>) {
  for (const g of pieces) {
    if (g.w <= 0 || g.h <= 0) throw new Error(`Descuento de vidrio inválido (${g.label}): ${g.w} x ${g.h} mm.`);
...
function kgAluminio(db: CostDB, i: QuoteInput): number {
  ...
  if (!items.length) throw new Error(`BOM vacío para ${i.linea}/${i.tipologia}/${i.variante}`);
...
function calcAcc(db: CostDB, a: Accesorio, meta: { per_ml: number; per_hoja_ml: number; hojas: number; peso_hoja_vidrio_kg?: number; ancho_mm: number; alto_mm: number }): { subtotal: number; cantidad: number } {
  ...
  if (a.regla === "por_abertura") return { subtotal: a.precio_ars * cantidadBase, cantidad: cantidadBase };
  ...
  if (a.regla === "clips_tapajunta") {
    ...
    const segs: number[] = esPuerta ? [meta.ancho_mm, meta.alto_mm, meta.alto_mm] : [meta.ancho_mm, meta.ancho_mm, meta.alto_mm, meta.alto_mm];
    const nSeg = (L: number) => {
      if (L <= 0) return 0;
      if (L <= 1000) return 2;
...
function calcAccesorios(db: CostDB, i: QuoteInput, meta: { per_ml: number; per_hoja_ml: number; hojas: number; peso_hoja_vidrio_kg?: number; ancho_mm: number; alto_mm: number }): { total: number; lines: any[] } {
  ...
  const mosq = (i as any).mosquitero ? db.accesorios.filter(a => a.tipologia === "MOSQUITEROS") : [];
...
function tapajuntaCosto(db: CostDB, i: QuoteInput, tap_ml: number): { total: number; lines: Array<{ codigo: string; descripcion: string; unidad: string; cantidad: number; unitario_ars: number; subtotal_ars: number }> } {
...
function vidrioPiezas(db: CostDB, i: QuoteInput): Array<{ label: string; ancho_mm: number; alto_mm: number; area_m2: number }> {
  const A = i.ancho_mm;
  const H = i.alto_mm;
...
function vidrioPiecesMm(db: CostDB, i: QuoteInput): Array<{w: number; h: number; label: string}> {
  if (i.tipologia === "CORREDIZA_2H") {
    const A = i.ancho_mm;
    const H = i.alto_mm;
    const w = (A/2) - Number(db.parametros.desc_corrediza_ancho_mm ?? 78);
