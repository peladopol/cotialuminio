import { QuoteInput, QuoteOutputAdmin } from "./types";
import { CostDB, Accesorio } from "./pricing";

const round2 = (n: number) => Math.round((n + Number.EPSILON) * 100) / 100;
const round3 = (n: number) => Math.round((n + Number.EPSILON) * 1000) / 1000;
const ceil = Math.ceil;

function areaM2(a_mm: number, h_mm: number) {
  return (a_mm / 1000) * (h_mm / 1000);
}
function perimetroMl(a_mm: number, h_mm: number) {
  return 2 * (a_mm / 1000 + h_mm / 1000);
}

function getMargenPct(db: CostDB, i: QuoteInput): number {
  const m = db.margenes.find(
    (x) =>
      x.linea === i.linea &&
      x.tipologia === i.tipologia &&
      x.canal === i.canal &&
      x.forma_pago === i.forma_pago
  );
...
function getVidrio(db: CostDB, id: string) {
  ...
  if (!v) throw new Error(`Vidrio no encontrado: ${raw}`);
  return v;
}

function getSeparadorPrecio(db: CostDB, camara_mm?: number): number {
  if (!camara_mm) return 0;
  const exact = db.separadores.find((x) => x.ancho_mm === camara_mm);
  if (exact) return exact.precio_ml_ars;
  if (db.separadores.length === 0) return 0;
  const best = db.separadores.reduce((b, c) =>
    Math.abs(c.ancho_mm - camara_mm) < Math.abs(b.ancho_mm - camara_mm) ? c : b
  );
  return best.precio_ml_ars;
}

function vidrioIdFromToken(db: CostDB, token: string): string {
  // Tokens posibles desde Excel/Widget: 4,5,33,L33,INC_4,LAM_33, etc.
  const tRaw = String(token || "")
    .trim()
    .toUpperCase();
  const t = normId(tRaw);
  ...
  if (t === "33" || t === "L33" || t === "LAM33") {
    const cand = db.vidrios.find(
      (v) =>
        v.tipo === "LAM" &&
        (/33|3_3/.test(normId(v.id)) ||
          String(v.configuracion ?? "")
            .toUpperCase()
            .includes("3+3"))
    );
    return cand?.id ?? "LAM_3_3";
  }
  ...
  if (/^\d+(\.\d+)?$/.test(tRaw.replace(",", "."))) {
    ...
    const cand = db.vidrios.find(
      (v) => v.tipo === "INC" && Math.abs((v.espesor_total_mm ?? 0) - mm) < 0.01
    );
    if (cand) return cand.id;
    // fallback común
    if (mm === 4) return "INC_4";
    if (mm === 5) return "INC_5";
  }
...
function parseDVHId(
  db: CostDB,
  dvhId: string
): { interior: string; exterior: string; camara_mm: number } {
  // Acepta:
  // - DVH_<ext>_<camara>_<int> (p.ej. DVH_4_9_4, DVH_L33_12_4, DVH_4_12_L33, DVH_L33_12_L33)
  // - DVH_3_3_<camara>_<int> (equivale a ext=L33)
  // - DVH_<ext>_<camara>_3_3 (equivale a int=L33)
  // - DVH_3_3_<camara>_3_3 (equivale a ext=L33 e int=L33)
  const parts = dvhId.split("_");
  if (parts[0] !== "DVH") throw new Error(`ID DVH inválido: ${dvhId}`);

  const readGlass = (idx: number): { token: string; next: number } => {
    // "3_3" => laminado 3+3 (L33)
    if (parts[idx] === "3" && parts[idx + 1] === "3")
      return { token: "L33", next: idx + 2 };
    return { token: parts[idx], next: idx + 1 };
  };

  let idx = 1;
  if (idx >= parts.length) throw new Error(`ID DVH inválido: ${dvhId}`);

  const ext = readGlass(idx);
  idx = ext.next;

  const camara_raw = parts[idx];
  const camara_mm = Number(camara_raw);
  if (!Number.isFinite(camara_mm)) throw new Error(`ID DVH inválido: ${dvhId}`);
  idx += 1;

  if (idx >= parts.length) throw new Error(`ID DVH inválido: ${dvhId}`);
  const inte = readGlass(idx);
  idx = inte.next;

  // no deben quedar tokens sueltos
  if (idx !== parts.length) throw new Error(`ID DVH inválido: ${dvhId}`);

  const exterior = vidrioIdFromToken(db, ext.token);
  const interior = vidrioIdFromToken(db, inte.token);

  return { interior, exterior, camara_mm };
}

function kg_m(db: CostDB, codigo: string): number {
  const v = db.perfiles.find((p) => p.codigo === codigo)?.kg_m;
  if (v == null) throw new Error(`Falta kg/m para perfil ${codigo}`);
  if (v <= 0)
    throw new Error(
      `kg/m inválido para perfil ${codigo}. Cargalo en Excel (hoja Perfiles).`
    );
  return v;
}
...
function kgPerfil(
  ...
  if (validateLength) {
    ...
    if (largo_mm > barra)
      throw new Error(
        `Corte de perfil ${codigo} = ${Math.round(
          largo_mm
        )} mm supera barra de ${barra} mm.`
      );
    if (largo_mm <= 0)
...
function validateRangos(db: CostDB, i: QuoteInput) {
  ...
  if (a < minA || h < minH)
    throw new Error(
      `Medida fuera de rango para ${i.tipologia}. Mínimo: ${minA} x ${minH} mm.`
    );
  if (a > maxA || h > maxH)
    throw new Error(
      `Medida fuera de rango para ${i.tipologia}. Máximo: ${maxA} x ${maxH} mm.`
    );
}

function fitsInGlassSheet(db: CostDB, w_mm: number, h_mm: number): boolean {
...
function validateVidrioSheet(
  ...
  for (const g of pieces) {
    if (g.w <= 0 || g.h <= 0)
      throw new Error(
        `Descuento de vidrio inválido (${g.label}): ${g.w} x ${g.h} mm.`
      );
    if (!fitsInGlassSheet(db, g.w, g.h)) {
      ...
      throw new Error(
        `Vidrio (${
          g.label
        }) no entra en plancha ${W}x${H} mm: pieza ${Math.round(
          g.w
        )}x${Math.round(g.h)} mm.`
      );
    }
...
function usoCorrediza(i: QuoteInput): "VENTANA" | "PUERTA" {
  return i.uso_corrediza === "PUERTA" ? "PUERTA" : "VENTANA";
}

/** CORREDIZA 2 HOJAS – Marco 2 guías (manual) */
function kgCorrediza2H(db: CostDB, i: QuoteInput): number {
  const A = i.ancho_mm;
  const H = i.alto_mm;
...
function kgVentanaAbrir1H(db: CostDB, i: QuoteInput): number {
  const A = i.ancho_mm;
  const H = i.alto_mm;
...
function kgAluminio(db: CostDB, i: QuoteInput): number {
  if (i.tipologia === "CORREDIZA_2H") return kgCorrediza2H(db, i);
  if (i.tipologia === "VENTANA_ABRIR_1H") return kgVentanaAbrir1H(db, i);

  const items = db.bom.filter(
    (x) =>
      x.linea === i.linea &&
      x.tipologia === i.tipologia &&
...
function calcAcc(
  ...
  if (
    a.regla === "por_perimetro_hojas_ml" ||
    a.regla === "por_perimetro_hoja_ml"
  ) {
  ...
  if (a.regla === "clips_tapajunta") {
    const esPuerta = (meta as any).uso === "PUERTA"; // lo inyectamos desde calcAccesorios
    const segs: number[] = esPuerta
      ? [meta.ancho_mm, meta.alto_mm, meta.alto_mm]
      : [meta.ancho_mm, meta.ancho_mm, meta.alto_mm, meta.alto_mm];
    const nSeg = (L: number) => {
      if (L <= 0) return 0;
      if (L <= 1000) return 2;
  ...
  if (a.regla && a.regla.startsWith("cada_")) {
    ...
    if (!Number.isFinite(mm) || mm <= 0) return { subtotal: 0, cantidad: 0 };
  ...
  if ((a as any).regla === "formula_personalizada") {
    const notas = String((a as any).notas || "");
...
function calcAccesorios(
...
): { total: number; lines: any[] } {
  const linea = (meta as any).linea ?? (i as any).linea ?? "CLASICA";
  ...
  const baseFiltrado = base.filter((a) => {
    const notas = String((a as any).notas || "");
    if (!(i as any).mosquitero && /MOSQ/i.test(notas)) return false;
    return true;
  });
  ...
  const lines = itemsAll
    .map((a) => {
      const metaLocal =
        a.tipologia === "MOSQUITEROS" ? { ...meta, hojas: 1 } : meta;
...
function tapajuntaML(i: QuoteInput, params: CostDB["parametros"]): number {
  if (!i.tapajunta) return 0;
  const a = i.ancho_mm / 1000;
  const h = i.alto_mm / 1000;
  const esPuerta =
    i.tipologia === "CORREDIZA_2H" && usoCorrediza(i) === "PUERTA";
  return esPuerta ? 2 * h + a : 2 * (a + h);
}

function tapajuntaCosto(
  db: CostDB,
  i: QuoteInput,
  tap_ml: number
): {
  ...
  if (tap_ml <= 0) return { total: 0, lines: [] };
  const ars_kg =
    db.parametros.aluminio_usd_kg * db.parametros.tipo_cambio_ars_usd;
  ...
  const countSeg = (Lmm: number) => {
    if (Lmm <= 0) return 0;
    if (Lmm <= 1000) return 2;
  ...
  const costoHora =
    db.manoObra.find((x) => x.linea === i.linea && x.tipologia === "TAPA_JUNTA")
      ?.costo_hora_ars ?? 0;
  ...
  const lines = [
    {
      codigo: "99206",
      descripcion: "Tapajunta (perfil)",
      unidad: "kg",
      cantidad: round2(kg),
      unitario_ars: round2(ars_kg),
      subtotal_ars: round2(costo_al),
    },
    {
      codigo: "E-066",
      descripcion: "Escuadra alineación tapajunta",
      unidad: "u",
      cantidad: n_e066,
      unitario_ars: round2(e066),
      subtotal_ars: round2(costo_e066),
    },
    {
      codigo: "T-096",
      descripcion: "Clip tapajunta",
      unidad: "u",
      cantidad: n_t096,
      unitario_ars: round2(t096),
      subtotal_ars: round2(costo_t096),
    },
    {
      codigo: "B-057",
      descripcion: "Burlete tapajunta",
      unidad: "ml",
      cantidad: round2(tap_ml),
      unitario_ars: round2(b057),
      subtotal_ars: round2(costo_b057),
    },
    {
      codigo: "MO-TAP",
      descripcion: "Mano de obra tapajunta",
      unidad: "hh",
      cantidad: round2(hh),
      unitario_ars: round2(costoHora),
      subtotal_ars: round2(costo_mo),
    },
  ];
...
function areaVidrioM2(db: CostDB, i: QuoteInput): number {
...
function vidrioPiezas(
  ...
  if (i.tipologia === "CORREDIZA_2H") {
    ...
    const h =
      H -
      (usoCorrediza(i) === "PUERTA"
        ? Number(db.parametros.desc_corrediza_alto_puerta_mm ?? 222)
        : Number(db.parametros.desc_corrediza_alto_ventana_mm ?? 167));
...
function vidrioPiecesMm(
  db: CostDB,
  i: QuoteInput
): Array<{ w: number; h: number; label: string }> {
  if (i.tipologia === "CORREDIZA_2H") {
    const A = i.ancho_mm;
    const H = i.alto_mm;
    const w = A / 2 - Number(db.parametros.desc_corrediza_ancho_mm ?? 78);
...
export function cotizar(db: CostDB, i: QuoteInput): QuoteOutputAdmin {
  const A = i.ancho_mm;
  const H = i.alto_mm;
  validateRangos(db, i);
  ...
  if (v.tipo === "DVH") {
    ...
    const vExt = getVidrio(db, exterior);
    espesor_vidrio_mm =
      (vInt.espesor_total_mm ?? 0) + (vExt.espesor_total_mm ?? 0);
  }
  const peso_hoja_vidrio_kg =
    (areaV / hojas) *
    espesor_vidrio_mm *
    db.parametros.peso_vidrio_kg_m2_mm *
    1.02; // +2%
  ...
  const accDet = calcAccesorios(db, i, {
    per_ml: per,
    per_hoja_ml: per_hoja,
    hojas,
    peso_hoja_vidrio_kg,
    ancho_mm: A,
    alto_mm: H,
  ...
  const moItems = db.manoObra.filter(
    (x) =>
      x.linea === i.linea &&
      x.tipologia === i.tipologia &&
      !x.etapa.startsWith("2-")
  );
  let mano_obra_ars = moItems.reduce(
    (s, x) => s + x.hh * (x.costo_hora_ars ?? 0),
    0
  );
