...
function areaM2(a_mm, h_mm) {
    return (a_mm / 1000) * (h_mm / 1000);
}
function perimetroMl(a_mm, h_mm) {
    return 2 * (a_mm / 1000 + h_mm / 1000);
}
function getMargenPct(db, i) {
    const m = db.margenes.find((x) => x.linea === i.linea &&
...
function vidrioIdFromToken(db, token) {
    ...
    if (t === "33" || t === "L33" || t === "LAM33") {
        const cand = db.vidrios.find((v) => v.tipo === "LAM" &&
...
function calcAcc(db, a, meta) {
    ...
    if (a.regla === "por_perimetro_hojas_ml" ||
...
function tapajuntaCosto(db, i, tap_ml) {
    ...
    const costoHora = db.manoObra.find((x) => x.linea === i.linea && x.tipologia === "TAPA_JUNTA")
        ?.costo_hora_ars ?? 0;
...
export function cotizar(db, i) {
    ...
    const peso_hoja_vidrio_kg = (areaV / hojas) *
        espesor_vidrio_mm *
    ...
    const moItems = db.manoObra.filter((x) => x.linea === i.linea &&
        x.tipologia === i.tipologia &&
    ...
    const costo_sin_iva = aluminio_ars +
        accesorios_ars +
        vidrio_ars +
        separador_ars +
        mano_obra_ars +
